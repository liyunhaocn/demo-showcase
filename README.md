...
当前 Demo 包括 `Actions Parallelism Planner`、`Context Split Planner`、`AGENTS.md Coverage Radar`、`AgentGate`、`Agent Trace Console`、`AnswerRank`、`CopilotBurn`、`MergePulse`、`MCP Gateway Readiness Scanner`、`ModelShift`、`Prompt Drift Lab`、`Release Note Impact Radar` 和 `Workspace Agent Route Planner`，都已内置在 `demos/<demo-id>/`，部署到 Pages 后可以直接从展示站打开演示。
