...
当前 Demo 包括 `Voice Turn QA Lab`、`Pipeline Agent Pilot Planner`、`Agent Setup Quarantine`、`Actions Parallelism Planner`、`Agent Repo Trust Scanner`、`Browser Tool Safety Planner`、`Code Quality Rollout Planner`、`Context Split Planner`、`AGENTS.md Coverage Radar`、`AgentGate`、`Agent Trace Console`、`AnswerRank`、`CopilotBurn`、`Issue Triage Field Planner`、`MergePulse`、`MCP Gateway Readiness Scanner`、`ModelShift`、`Prompt Drift Lab`、`Release Note Impact Radar`、`Secret Exposure Response Planner` 和 `Workspace Agent Route Planner`，都已内置在 `demos/<demo-id>/`，部署到 Pages 后可以直接从展示站打开演示。
