...
function bindEvents() {
  ...
  elements.resetButton.addEventListener('click', () => {
    const confirmed = window.confirm('Reset demo data and clear the saved Prompt Drift Lab brief from this browser?')
    if (!confirmed) return
    state.brief = null
    state.copyFallbackVisible = false
    clearErrors()
  ...
  elements.copyButton.addEventListener('click', async () => {
    ...
    if (result.ok) {
      ...
      state.lastMessage = `Clipboard copy unavailable. ${result.reason || 'Use the drift panel.'}`
    }
    renderReport()
  })
  ...
  elements.driftButton.addEventListener('click', () => {
    state.copyFallbackVisible = true
    state.lastMessage = 'Markdown drift opened for manual copy.'
    renderReport()
  })
...
function generateFromForm() {
  ...
  submit.textContent = 'Building deterministic drift brief...'
...
function readForm() {
  return {
    ...
    objective: document.querySelector('input[name="objective"]:checked')?.value || 'migrate-safely',
    driftModels: elements.driftModels.value
      .split('\n')
      .map((line, index) => parseFallbackModel(line, index))
      .filter(Boolean),
  }
}

function parseFallbackModel(line, index) {
  const raw = line.trim()
  if (!raw) return null
...
function fillForm(profile) {
  ...
  const objectiveInput = document.querySelector(`input[name="objective"][value="${profile.objective || 'migrate-safely'}"]`)
...
function renderReport() {
  ...
  replaceChildren(elements.scoreGrid, [
    metricCard(`${brief.score.overall}`, `Overall drift readiness - ${scoreBand(brief.score.overall)}`),
    metricCard(`${brief.findings.length}`, 'workflows tested'),
    metricCard(`${brief.score.driftReadiness}`, 'drift readiness'),
    metricCard(`${brief.score.regressionCoverage}`, 'regression coverage'),
  ])

  replaceChildren(elements.summaryGrid, [
    summaryCard('Prompt snapshot', `${brief.profile.appName} comparing baseline and candidate prompts across ${brief.profile.region}.`),
    summaryCard('Drift gap', brief.summary.driftGap),
    summaryCard('Next week focus', `${copy.framing} Priority: ${brief.summary.topAction}`),
  ])
...
function promptCard(brief, finding) {
  ...
  card.append(
    cell('Critical flow', prompt?.prompt || finding.promptId),
    statusCell(finding.status),
    cell('Intent', INTENT_GROUPS[prompt?.intentGroup] || prompt?.intentGroup || ''),
    cell('Drift gap', SOURCE_GAPS[finding.likelySourceGap] || finding.likelySourceGap),
    cell('Recommended action', finding.recommendedAction),
  )
  if (finding.topAlternatives?.length) {
    card.append(cell('Fallback pressure', finding.topAlternatives.join(', ')))
  }
  return card
}
...
function setGenerateLabel() {
  const submit = elements.form.querySelector('button[type="submit"]')
  if (!submit.disabled) submit.textContent = state.brief ? 'Regenerate drift brief' : 'Generate drift brief'
}
