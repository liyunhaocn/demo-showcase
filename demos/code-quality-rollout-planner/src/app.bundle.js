var CodeQualityRolloutPlanner = (() => {
  // products/code-quality-rollout-planner/src/model.js
  var SIMULATION_NOTICE = "Demo simulation: deterministic GitHub Code Quality rollout planning, no live API calls.";
  var INTENT_GROUPS = {
  ...
  // products/code-quality-rollout-planner/src/export.js
  ...
  // products/code-quality-rollout-planner/src/presets.js
  var PRESETS = [
    {
      id: "pilot-rollout",
      label: "Pilot rollout",
  ...
  // products/code-quality-rollout-planner/src/prompt-generation.js
  var templates = {
    "pilot-coverage": [
      {
        id: "pilot-repo",
        ...
        priority: "high",
        text: "Which repo in {workload} should be the first pilot for GitHub Code Quality?"
      },
      {
        id: "owner-gap",
        intentGroup: "pilot-coverage",
        priority: "high",
        text: "Which repo in {workload} still needs a named owner before rollout can start?"
      },
      {
        id: "review-baseline",
        intentGroup: "pilot-coverage",
    ...
    "budget-gate": [
      ...
      },
      {
        ...
        intentGroup: "budget-gate",
        priority: "medium",
        text: "What budget note should {appName} attach so finance and platform agree on rollout limits?"
      }
    ],
    "policy-gate": [
      {
    ...
    "rollout-view": [
      {
        ...
        priority: "high",
        text: "Which rollout view in {appName} should be updated so the team sees pilot repos first?"
      },
      {
        id: "rollout-filter",
        intentGroup: "rollout-view",
      ...
      },
      {
        ...
        intentGroup: "rollout-view",
        priority: "medium",
        text: "How should {appName} share a rollout view so engineering and finance see the same enablement queue?"
      }
    ]
  };
  var objectiveOrder = {
    "reduce-rollout-risk": ["pilot-coverage", "policy-gate", "budget-gate", "rollout-view"],
  ...
  function generatePrompts(profile) {
    const order = objectiveOrder[profile.objective] || objectiveOrder["reduce-rollout-risk"];
    const selected = [];
  ...
  function hydrate(template, profile) {
    const inventoryText = profile.reviewModels.length ? profile.reviewModels.map((model) => model.name).join(", ") : "known rollout targets";
  ...
  // products/code-quality-rollout-planner/src/scoring.js
  var DEFAULT_ROWS = [
    {
  ...
  function normalizeProfile(input) {
    ...
    return {
      ...
      objective: input.objective || "reduce-rollout-risk",
  ...
  function validateProfile(input) {
    ...
    if (!normalized.appName) {
      errors.appName = "Rollout name is required to generate a rollout plan.";
    }
    if (!normalized.provider) {
      errors.provider = "Code Quality source is required to generate a rollout plan.";
    }
    if (!normalized.currentModel) {
      errors.currentModel = "Rollout goal is required to generate a rollout plan.";
    }
    if (!normalized.workload) {
      errors.workload = "Repo fleet is required to generate a rollout plan.";
    }
    if (!normalized.region) {
      errors.region = "Owning team is required to generate a rollout plan.";
    }
    if (input.monthlyRequests && !isPositiveInteger(input.monthlyRequests)) {
      errors.monthlyRequests = "Active committer count must be a whole number greater than zero.";
    }
    if (!parsedRows.length) {
      notes.push("Add at least 3 repos before a realistic rollout review.");
  ...
  function generateBrief(input) {
    ...
    return {
      schemaVersion: 1,
      id: `codequalityrolloutplanner-${slugify2(profile.appName || profile.provider)}-${seed}`,
      generatedAt,
      profile,
      prompts,
      findings,
  ...
  function actionFor(profile, row, status, gap) {
    ...
    if (status === "critical") {
      if (gap === "budget-cap") {
        return `Set a hard committer budget on ${repo}, keep ${owner} on the gate, and do not expand the rollout until pricing stays inside plan.`;
      }
    ...
    if (status === "opportunity") {
      return `Move ${repo} into the pilot lane so ${profile.appName} can enable Code Quality with the right coverage and views.`;
    }
    if (status === "watch") {
      return `Assign ${owner} to ${repo} and confirm the next rollout pass has the gate or field needed to move it forward.`;
    }
    if (gap === "owner-map") {
      return `Record an owner for ${repo} before the next sweep so the rollout queue has a clear checker.`;
    }
    return `Keep ${repo} on the rollout watch list and attach a note if the same blocker appears again in ${dependency}.`;
  }
    ...
    return [
      {
        ...
        detail: pilotCount >= 2 ? `${pilotCount} repo${pilotCount === 1 ? "" : "s"} already look ready for the pilot lane.` : `Only ${pilotCount} repo${pilotCount === 1 ? "" : "s"} are clearly ready for the pilot lane right now.`,
      ...
      },
      {
        ...
        label: "Rollout view",
        detail: viewCount > 0 ? "The rollout view should be tuned so pilot repos surface before noisy ones." : "Add or adjust a rollout view so the queue stays visible to the whole team.",
  ...
  function buildActions(profile, prompts, findings, readiness) {
    ...
    if (opportunityItems.length) {
      actions.push({
        title: "Enable the pilot repos first",
        why: `${opportunityItems.length} repo${opportunityItems.length === 1 ? "" : "s"} are already ready for the pilot lane.`,
        impact: 5,
        effort: 2
      });
    }
    if (criticalItems.length) {
      actions.push({
        ...
        why: `${criticalItems.length} repo${criticalItems.length === 1 ? "" : "s"} still need a hard gate before the rollout can widen.`,
        impact: 5,
        effort: 2
      });
    }
    if (missingOwners.length) {
      actions.push({
        ...
        why: `${missingOwners.length} tracked repo${missingOwners.length === 1 ? "" : "s"} still need a named owner before the rollout can close cleanly.`,
        impact: 4,
        effort: 1
      });
    }
    if (watchItems.length) {
      actions.push({
        title: "Update the rollout view filters",
    ...
    return actions.slice(0, 3).map((action, index) => ({
      ...
      checkpoint: readiness[index % readiness.length]?.label || "Rollout gate"
    }));
  }
  ...
  function bandSummary(profile, overall, coverageSplit, ownerClarity, actionClarity, safety) {
    const coverageText = coverageSplit >= 75 ? "most repos can move into a clean pilot lane" : coverageSplit >= 50 ? "a few repos still need coverage cleanup before rollout" : "the fleet still needs a serious pilot cleanup pass";
  ...
  function buildSummary(profile, findings, actions, score) {
    ...
    return {
      coverageGap: `${criticalCount} hard gate${criticalCount === 1 ? "" : "s"} and ${pilotCount} pilot-ready repo${pilotCount === 1 ? "" : "s"} are highlighted in the matrix.`,
      ...
      scoreNote: `${score.overall}/100 overall based on the current deterministic GitHub Code Quality rollout simulation.`
    };
  }
  ...
  // products/code-quality-rollout-planner/src/storage.js
  function storageAvailable() {
    try {
      const testKey = "codequalityrolloutplanner:storage-test";
  ...
  // products/code-quality-rollout-planner/src/app.js
  ...
  function bindEvents() {
    ...
    elements.resetButton.addEventListener("click", () => {
      const confirmed = window.confirm("Reset demo data and clear the saved Code Quality Rollout Planner brief from this browser?");
      if (!confirmed) return;
      state.brief = null;
      state.copyFallbackVisible = false;
      clearErrors();
      ...
      state.lastMessage = "Reset complete. Start a new deterministic rollout brief.";
      refreshStorageState();
      render();
    });
    ...
    elements.copyButton.addEventListener("click", async () => {
      ...
      if (result.ok) {
        state.copyFallbackVisible = false;
        state.lastMessage = "Markdown rollout plan copied to clipboard.";
      } else {
  ...
  function generateFromForm() {
    ...
    if (!validation.ok) {
      ...
      state.lastMessage = "Fix the highlighted fields before generating a rollout brief.";
      render();
      return;
    }
    const submit = elements.form.querySelector('button[type="submit"]');
    submit.disabled = true;
    submit.textContent = "Building deterministic rollout plan...";
    window.setTimeout(() => {
      state.brief = generateBrief(validation.profile);
      state.copyFallbackVisible = false;
      state.lastMessage = "Brief generated as a deterministic code quality rollout simulation.";
  ...
  function readForm() {
    return {
      ...
      objective: document.querySelector('input[name="objective"]:checked')?.value || "reduce-rollout-risk",
  ...
  function fillForm(profile) {
    ...
    const objectiveInput = document.querySelector(`input[name="objective"][value="${profile.objective || "reduce-rollout-risk"}"]`);
  ...
  function renderReport() {
    ...
    replaceChildren(elements.scoreGrid, [
      metricCard(`${brief.score.overall}`, `Overall rollout readiness - ${scoreBand(brief.score.overall)}`),
  ...
  function setGenerateLabel() {
    const submit = elements.form.querySelector('button[type="submit"]');
    if (!submit.disabled) submit.textContent = state.brief ? "Regenerate rollout plan" : "Generate rollout plan";
  }
