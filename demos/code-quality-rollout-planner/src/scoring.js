...
export function generateBrief(input) {
  ...
  return {
    schemaVersion: 1,
    id: `codequalityrolloutplanner-${slugify(profile.appName || profile.provider)}-${seed}`,
    generatedAt,
    profile,
    prompts,
    findings,
  ...
  return [
    {
      ...
      detail: pilotCount >= 2
        ? `${pilotCount} repo${pilotCount === 1 ? '' : 's'} already look ready for the pilot lane.`
        : `Only ${pilotCount} repo${pilotCount === 1 ? '' : 's'} are clearly ready for the pilot lane right now.`,
    ...
    },
    {
      ...
      label: 'Rollout view',
      detail: viewCount > 0
        ? 'The rollout view should be tuned so pilot repos surface before noisy ones.'
        : 'Add or adjust a rollout view so the queue stays visible to the whole team.',
