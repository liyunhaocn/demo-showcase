...
export function validateProfile(input) {
  ...
  if (!normalized.appName) {
    errors.appName = 'App name is required to generate a gateway brief.'
  }

  if (!normalized.provider) {
    errors.provider = 'Gateway source is required to generate a gateway brief.'
  }

  if (!normalized.currentModel) {
    errors.currentModel = 'Gateway headline is required to generate a gateway brief.'
  }

  if (!normalized.workload) {
    errors.workload = 'Tool surface is required to generate a gateway brief.'
  }

  if (!normalized.region) {
    errors.region = 'Owner group is required to generate a gateway brief.'
  }

  if (input.monthlyRequests && !isPositiveInteger(input.monthlyRequests)) {
    errors.monthlyRequests = 'Tracked servers must be a whole number greater than zero.'
  }

  if (!parsedRows.length) {
    notes.push('Add at least 3 gateway rows before a real gateway review.')
  } else if (parsedRows.length < 6) {
    notes.push('Add up to 6 gateway rows to exercise the full gateway matrix.')
  } else if (parsedRows.length > 6) {
    notes.push('Only the first 6 gateway rows are used in this deterministic demo.')
  }

  const missingOwnerRows = parsedRows.filter((row) => !row.owner)
  if (missingOwnerRows.length) {
    notes.push('Owner names are optional, but the matrix is clearer when each gateway row has an owner.')
  }
...
export function generateBrief(input) {
  ...
  return {
    schemaVersion: 1,
    id: `mcpgatewayreadinessscanner-${slugify(profile.appName || profile.provider)}-${seed}`,
    generatedAt,
    profile,
    prompts,
    findings,
...
function classifyStatus(profile, row, bucket) {
  ...
  if (containsAny(text, ['gateway', 'span', 'tool', 'handoff', 'approval', 'agent', 'eval', 'observability'])) {
...
function pickSourceGap(status, row, bucket) {
  ...
  if (status === 'watch') {
    ...
    return bucket % 2 === 0 ? 'span-inventory' : 'gateway-source'
  }
...
function actionFor(profile, row, status, gap) {
  ...
  const dependency = row.dependency || 'the gateway'

  if (status === 'critical') {
    return `Assign ${owner} to replay ${featureName} against ${dependency} and keep a fallback path ready for ${profile.appName}.`
  }
  if (status === 'opportunity') {
    return `Have ${owner} test whether ${featureName} can benefit from ${dependency} without changing the gateway gate.`
  }
  if (status === 'watch') {
    return `Monitor ${featureName} for the next gateway run and confirm the owner can smoke test the change.`
  }
  if (gap === 'owner-map') {
    return `Record ${owner} for ${featureName} so the next gateway has a clear checker.`
  }
  return `Keep ${featureName} on the watch list and note the gateway source in the team checklist.`
}
  ...
  return [
    {
      ...
      detail: profile.provider
        ? `The team has a named gateway source: ${profile.provider}.`
        : 'No gateway source has been mapped yet.',
      gap: profile.provider ? '' : 'gateway-source',
    },
    {
      ...
      detail:
        inventoryCount >= 6
          ? `The matrix covers ${inventoryCount} gateway rows.`
          : inventoryCount >= 3
            ? `The matrix has ${inventoryCount} rows, but 6 gives better coverage.`
            : 'The matrix is too small to expose the main gateway edges.',
    ...
    },
    {
      ...
      detail:
        criticalCount <= 1
          ? 'The critical paths can be smoke-tested before the next gateway run.'
          : 'The gateway needs a replay or smoke plan before it is safe.',
...
function buildActions(profile, prompts, findings, readiness) {
  ...
  return [
    {
      id: 'map-source',
      title: 'Map the gateway source',
      impact: 5,
      effort: 1,
      why: `Summarize ${profile.provider} into one gateway brief before the team acts on it.`,
    },
    {
      id: 'assign-owners',
      ...
      why: criticalRows.length
        ? `Make ${criticalRows.length} critical gateway rows explicit so the checklist has one owner per item.`
    ...
    {
      id: 'run-smoke',
      title: 'Run a gateway smoke check',
      impact: 4,
      effort: 2,
      why: `Use the gateway matrix to validate ${profile.workload} before the next gateway lands.`,
    },
    {
      id: 'capture-upgrade',
    ...
    },
    {
      ...
      impact: 4,
      effort: 1,
      why: 'Give the team one page that lists the gateway source, the matrix, the owners, and the next checks.',
    },
  ]
}
...
function buildSummary(profile, findings, actions, score) {
  ...
  const topAction = actions[0]?.title || 'Map the gateway source'
  const highestPressure = profile.driftModels[0]?.name || profile.currentModel

  return {
    headline:
      score.overall >= 70
        ? 'The gateway plan is manageable if the team keeps the matrix and owner checklist first.'
        : 'The gateway still has a few high-risk edges that should be fixed before cutover.',
    gatewayWindow:
      criticalCount > 0
        ? 'This week'
        : watchCount > 2
    ...
    impactGap:
      profile.driftModels.length
        ...
        : 'No gateway rows have been added yet, so the matrix is incomplete.',
    ownerGap:
      profile.driftModels.filter((row) => row.owner).length
        ? 'Owners are mapped for the gateway checklist.'
        : 'The gateway has no owner map yet.',
    topAction,
  }
}
