...
const elements = {
  ...
  fallbackModels: document.querySelector('#fallback-models'),
  fallbackHint: document.querySelector('#fallback-hint'),
...
function bindEvents() {
  ...
  elements.resetButton.addEventListener('click', () => {
    const confirmed = window.confirm('Reset demo data and clear the saved ModelShift brief from this browser?')
    if (!confirmed) return
    state.brief = null
    state.copyFallbackVisible = false
    clearErrors()
...
function generateFromForm() {
  ...
  submit.textContent = 'Building deterministic migration brief...'
...
function readForm() {
  return {
    ...
    fallbackModels: elements.fallbackModels.value
      .split('\n')
...
function fillForm(profile) {
  ...
  elements.fallbackModels.value = (profile.fallbackModels || [])
...
function renderReport() {
  ...
  replaceChildren(elements.scoreGrid, [
    metricCard(`${brief.score.overall}`, `Overall migration readiness - ${scoreBand(brief.score.overall)}`),
    metricCard(`${brief.findings.length}`, 'workflows tested'),
    metricCard(`${brief.score.fallbackReadiness}`, 'fallback readiness'),
    metricCard(`${brief.score.regressionCoverage}`, 'regression coverage'),
  ])

  replaceChildren(elements.summaryGrid, [
    summaryCard('Stack snapshot', `${brief.profile.appName} using ${brief.profile.provider} / ${brief.profile.currentModel} across ${brief.profile.region}.`),
    summaryCard('Fallback gap', brief.summary.fallbackGap),
...
function promptCard(brief, finding) {
  ...
  card.append(
    ...
    cell('Migration gap', SOURCE_GAPS[finding.likelySourceGap] || finding.likelySourceGap),
...
function renderValidation(validation) {
  ...
  elements.fallbackHint.textContent = validation.notes.length
...
function setGenerateLabel() {
  const submit = elements.form.querySelector('button[type="submit"]')
  if (!submit.disabled) submit.textContent = state.brief ? 'Regenerate migration brief' : 'Generate migration brief'
}
