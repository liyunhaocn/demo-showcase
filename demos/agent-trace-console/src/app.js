...
function bindEvents() {
  ...
  elements.resetButton.addEventListener('click', () => {
    const confirmed = window.confirm('Reset demo data and clear the saved Agent Trace Console brief from this browser?')
    if (!confirmed) return
    state.brief = null
    state.copyFallbackVisible = false
    clearErrors()
  ...
  elements.copyButton.addEventListener('click', async () => {
    ...
    if (result.ok) {
      state.copyFallbackVisible = false
      state.lastMessage = 'Markdown trace brief copied to clipboard.'
    } else {
...
function generateFromForm() {
  ...
  submit.textContent = 'Building deterministic trace brief...'
...
function renderReport() {
  ...
  replaceChildren(elements.scoreGrid, [
    metricCard(`${brief.score.overall}`, `Overall trace readiness - ${scoreBand(brief.score.overall)}`),
    metricCard(`${brief.findings.length}`, 'trace items'),
    metricCard(`${brief.score.impactCoverage}`, 'trace coverage'),
    metricCard(`${brief.score.ownerClarity}`, 'owner clarity'),
  ])

  replaceChildren(elements.summaryGrid, [
    summaryCard('Trace snapshot', `${brief.profile.appName} comparing ${brief.profile.provider} and the tracked spans across ${brief.profile.region}.`),
    summaryCard('Trace gap', brief.summary.impactGap),
...
function promptCard(brief, finding) {
  ...
  card.append(
    cell('Trace item', prompt?.prompt || finding.promptId),
...
function setGenerateLabel() {
  const submit = elements.form.querySelector('button[type="submit"]')
  if (!submit.disabled) submit.textContent = state.brief ? 'Regenerate trace brief' : 'Generate trace brief'
}
