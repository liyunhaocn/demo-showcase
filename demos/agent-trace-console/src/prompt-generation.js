  ...
  'trace-source': [
    {
      ...
      intentGroup: 'trace-source',
      priority: 'high',
      text: 'Which trace source in {appName} shows the slowest or noisiest run first?',
    },
    {
      id: 'trace-source-depth',
      intentGroup: 'trace-source',
      priority: 'high',
      text: 'Which spans in {workload} need more detail before the next incident review?',
    },
    {
      id: 'trace-source-warning',
      intentGroup: 'trace-source',
      priority: 'medium',
      text: 'How much signal does {appName} need before a trace becomes a trace blocker?',
    },
  ],
  'failure-mode': [
    {
    ...
    },
    {
      ...
      intentGroup: 'failure-mode',
      priority: 'medium',
      text: 'Which trace step should {appName} replay first to reproduce the problem?',
    },
  ],
  'handoff-gap': [
    {
  ...
  'replay-plan': [
    {
      ...
      priority: 'high',
      text: 'Should {appName} keep the current trace path or replay a safer path first?',
    },
    {
      id: 'replay-fit',
      intentGroup: 'replay-plan',
      priority: 'medium',
      text: 'Which owner best understands {workload} if the team cares most about trace clarity?',
    },
    {
      id: 'replay-sequence',
      intentGroup: 'replay-plan',
      priority: 'medium',
      text: 'What replay sequence reduces trace risk fastest for {appName}?',
    },
  ],
}

const objectiveOrder = {
  'find-bottlenecks': ['trace-source', 'failure-mode', 'handoff-gap', 'replay-plan'],
  'reduce-failures': ['failure-mode', 'handoff-gap', 'trace-source', 'replay-plan'],
  'reduce-cost': ['trace-source', 'replay-plan', 'failure-mode', 'handoff-gap'],
}
...
function hydrate(template, profile) {
  ...
  return template
    ...
    .replaceAll('{workload}', profile.workload || 'the current trace surface')
