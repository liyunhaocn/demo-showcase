...
const output = path.join(showcaseRoot, 'demos', 'agent-repo-trust-scanner')
...
execFileSync(
  ...
  [
    ...
    '--format=iife',
    '--platform=browser',
    '--global-name=AgentRepoTrustScanner',
    `--outfile=${bundlePath}`,
  ],
  { stdio: 'inherit' },
)
...
console.log(`Built Agent Repo Trust Scanner static demo into ${path.relative(showcaseRoot, output)}`)
