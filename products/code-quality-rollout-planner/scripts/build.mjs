...
const output = path.join(showcaseRoot, 'demos', 'code-quality-rollout-planner')
...
execFileSync(
  ...
  [
    ...
    '--format=iife',
    '--platform=browser',
    '--global-name=CodeQualityRolloutPlanner',
    `--outfile=${bundlePath}`,
  ],
  { stdio: 'inherit' },
)
...
console.log(`Built Code Quality Rollout Planner static demo into ${path.relative(showcaseRoot, output)}`)
