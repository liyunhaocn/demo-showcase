...
const output = path.join(showcaseRoot, 'demos', 'agents-md-coverage-radar')
...
execFileSync(
  ...
  [
    ...
    '--format=iife',
    '--platform=browser',
    '--global-name=AgentsMdCoverageRadar',
    `--outfile=${bundlePath}`,
  ],
  { stdio: 'inherit' },
)
...
console.log(`Built AGENTS.md Coverage Radar static demo into ${path.relative(showcaseRoot, output)}`)
