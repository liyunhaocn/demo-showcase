...
export function generateBrief(input) {
  ...
  return {
    schemaVersion: 1,
    id: `agentsmdcoverageradar-${slugify(profile.appName || profile.provider)}-${seed}`,
    generatedAt,
    profile,
    prompts,
    findings,
  ...
  return [
    {
      ...
      detail: profile.provider
        ? `The team has a named instruction source: ${profile.provider}.`
        : 'No instruction source has been mapped yet.',
    ...
    },
    {
      ...
      detail:
        inventoryCount >= 6
          ? `The matrix covers ${inventoryCount} coverage rows.`
          : inventoryCount >= 3
            ? `The matrix has ${inventoryCount} rows, but 6 gives better coverage.`
            : 'The matrix is too small to expose the main coverage edges.',
    ...
    },
    {
      ...
      detail:
        criticalCount <= 1
          ? 'The critical paths can be smoke-tested before the next coverage run.'
          : 'The coverage needs a replay or smoke plan before it is safe.',
  ...
  return [
    ...
    },
    {
      ...
      why: criticalRows.length
        ? `Make ${criticalRows.length} critical coverage rows explicit so the checklist has one owner per item.`
...
function buildSummary(profile, findings, actions, score) {
  ...
  return {
    headline:
      score.overall >= 70
        ? 'The coverage plan is manageable if the team keeps the matrix and owner checklist first.'
        : 'The coverage still has a few high-risk edges that should be fixed before cutover.',
    coverageWindow:
      criticalCount > 0
        ? 'This week'
        : watchCount > 2
    ...
    impactGap:
      profile.reviewModels.length
        ...
        : 'No coverage rows have been added yet, so the matrix is incomplete.',
    ownerGap:
      profile.reviewModels.filter((row) => row.owner).length
        ? 'Owners are mapped for the instruction checklist.'
        : 'The coverage has no owner map yet.',
    topAction,
  }
}
