...
function bindEvents() {
  ...
  elements.resetButton.addEventListener('click', () => {
    const confirmed = window.confirm('Reset demo data and clear the saved Release Note Impact Radar brief from this browser?')
    if (!confirmed) return
    state.brief = null
    state.copyFallbackVisible = false
    clearErrors()
  ...
  elements.copyButton.addEventListener('click', async () => {
    ...
    if (result.ok) {
      state.copyFallbackVisible = false
      state.lastMessage = 'Markdown impact brief copied to clipboard.'
    } else {
      state.copyFallbackVisible = true
      state.lastMessage = `Clipboard copy unavailable. ${result.reason || 'Use the impact panel.'}`
    }
    renderReport()
  })
...
function generateFromForm() {
  ...
  submit.disabled = true
    submit.textContent = 'Building deterministic impact brief...'
...
function renderReport() {
  ...
  replaceChildren(elements.scoreGrid, [
    metricCard(`${brief.score.overall}`, `Overall impact readiness - ${scoreBand(brief.score.overall)}`),
    metricCard(`${brief.findings.length}`, 'feature rows'),
    metricCard(`${brief.score.impactCoverage}`, 'impact coverage'),
    metricCard(`${brief.score.ownerClarity}`, 'owner clarity'),
  ])

  replaceChildren(elements.summaryGrid, [
    summaryCard('Release snapshot', `${brief.profile.appName} comparing ${brief.profile.provider} and the tracked feature rows across ${brief.profile.region}.`),
    summaryCard('Impact gap', brief.summary.impactGap),
...
function setGenerateLabel() {
  const submit = elements.form.querySelector('button[type="submit"]')
  if (!submit.disabled) submit.textContent = state.brief ? 'Regenerate impact brief' : 'Generate impact brief'
}
