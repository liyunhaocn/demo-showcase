...
const elements = {
  ...
  driftModels: document.querySelector('#drift-models'),
  driftHint: document.querySelector('#drift-hint'),
  ...
  driftButton: document.querySelector('#drift-button'),
  driftPanel: document.querySelector('#drift-panel'),
  driftMarkdown: document.querySelector('#drift-markdown'),
  selectFallback: document.querySelector('#select-drift-button'),
  closeFallback: document.querySelector('#close-drift-button'),
...
function bindEvents() {
  ...
  elements.restoreButton.addEventListener('click', () => {
    ...
    if (loaded.ok && loaded.value?.schemaVersion === 1) {
      ...
      state.lastMessage = 'Restored last plan from this browser.'
  ...
  elements.resetButton.addEventListener('click', () => {
    const confirmed = window.confirm('Reset demo data and clear the saved Workspace Agent Route Planner plan from this browser?')
    if (!confirmed) return
    state.brief = null
    state.copyFallbackVisible = false
    clearErrors()
    ...
    state.lastMessage = 'Reset complete. Start a new deterministic route plan.'
    refreshStorageState()
    render()
  })
  ...
  elements.copyButton.addEventListener('click', async () => {
    ...
    if (result.ok) {
      state.copyFallbackVisible = false
      state.lastMessage = 'Markdown route plan copied to clipboard.'
    } else {
      state.copyFallbackVisible = true
      state.lastMessage = `Clipboard copy unavailable. ${result.reason || 'Use the route panel.'}`
    }
    renderReport()
  })
  ...
  elements.driftButton.addEventListener('click', () => {
    state.copyFallbackVisible = true
  ...
  elements.selectFallback.addEventListener('click', () => {
    elements.driftMarkdown.focus()
    elements.driftMarkdown.select()
  })
...
function generateFromForm() {
  ...
  if (!validation.ok) {
    ...
    state.lastMessage = 'Fix the highlighted fields before generating a route plan.'
    render()
    return
  }
  ...
  submit.disabled = true
    submit.textContent = 'Building deterministic route plan...'

  window.setTimeout(() => {
    state.brief = generateBrief(validation.profile)
    state.copyFallbackVisible = false
    state.lastMessage = 'Route plan generated as a deterministic demo simulation.'
    if (!state.storageDisabled) {
      const saveResult = saveLastBrief(state.brief)
      if (!saveResult.ok) {
        state.storageDisabled = true
        state.lastMessage = 'Route plan generated. Storage is unavailable, so restore is disabled.'
      }
    }
    renderValidation(validation)
    refreshStorageState()
...
function readForm() {
  return {
    ...
    objective: document.querySelector('input[name="objective"]:checked')?.value || 'prevent-breakage',
    driftModels: elements.driftModels.value
      .split('\n')
...
function fillForm(profile) {
  ...
  const objectiveInput = document.querySelector(`input[name="objective"][value="${profile.objective || 'prevent-breakage'}"]`)
  if (objectiveInput) objectiveInput.checked = true
  elements.driftModels.value = (profile.driftModels || [])
    .map((drift) => {
      if (typeof drift === 'string') return drift
      return [drift.name, drift.dependency, drift.owner, drift.risk].filter(Boolean).join(' | ')
    })
    .join('\n')
  saveDraft()
  setGenerateLabel()
...
function renderReport() {
  ...
  const copy = VIEW_COPY[state.view] || VIEW_COPY.operator
  ...
  replaceChildren(elements.scoreGrid, [
    metricCard(`${brief.score.overall}`, `Overall route readiness - ${scoreBand(brief.score.overall)}`),
    metricCard(`${brief.findings.length}`, 'workflow rows'),
    metricCard(`${brief.score.impactCoverage}`, 'route coverage'),
    metricCard(`${brief.score.ownerClarity}`, 'owner clarity'),
  ])

  replaceChildren(elements.summaryGrid, [
    summaryCard('Route snapshot', `${brief.profile.appName} comparing ${brief.profile.provider} and the tracked workflow rows across ${brief.profile.region}.`),
    summaryCard('Route gap', brief.summary.impactGap),
    summaryCard('Next rollout focus', `${copy.framing} Priority: ${brief.summary.topAction}`),
  ])
  ...
  elements.driftMarkdown.value = markdown
  elements.driftPanel.hidden = !state.copyFallbackVisible
...
function promptCard(brief, finding) {
  ...
  card.append(
    cell('Feature', prompt?.prompt || finding.promptId),
...
function actionCard(action) {
  ...
  impact.textContent = `Lift ${action.impact}/5`
...
function renderValidation(validation) {
  ...
  elements.driftHint.textContent = validation.notes.length
...
function setGenerateLabel() {
  const submit = elements.form.querySelector('button[type="submit"]')
  if (!submit.disabled) submit.textContent = state.brief ? 'Regenerate route plan' : 'Generate route plan'
}
