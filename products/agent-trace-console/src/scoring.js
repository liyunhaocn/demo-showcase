...
export function validateProfile(input) {
  ...
  if (!normalized.appName) {
    errors.appName = 'App name is required to generate a trace brief.'
  }

  if (!normalized.provider) {
    errors.provider = 'Trace source is required to generate a trace brief.'
  }

  if (!normalized.currentModel) {
    errors.currentModel = 'Run headline is required to generate a trace brief.'
  }

  if (!normalized.workload) {
    errors.workload = 'Trace surface is required to generate a trace brief.'
  }

  if (!normalized.region) {
    errors.region = 'Owner group is required to generate a trace brief.'
  }

  if (input.monthlyRequests && !isPositiveInteger(input.monthlyRequests)) {
    errors.monthlyRequests = 'Tracked spans must be a whole number greater than zero.'
  }

  if (!parsedRows.length) {
    notes.push('Add at least 3 trace rows before a real trace review.')
  } else if (parsedRows.length < 6) {
    notes.push('Add up to 6 trace rows to exercise the full trace matrix.')
  } else if (parsedRows.length > 6) {
    notes.push('Only the first 6 trace rows are used in this deterministic demo.')
  }

  const missingOwnerRows = parsedRows.filter((row) => !row.owner)
  if (missingOwnerRows.length) {
    notes.push('Owner names are optional, but the matrix is clearer when each trace row has an owner.')
  }
...
export function generateBrief(input) {
  ...
  return {
    schemaVersion: 1,
    id: `agenttraceconsole-${slugify(profile.appName || profile.provider)}-${seed}`,
    generatedAt,
    profile,
    prompts,
    findings,
...
function classifyStatus(profile, row, bucket) {
  ...
  if (containsAny(text, ['trace', 'span', 'tool', 'handoff', 'approval', 'agent', 'eval', 'observability'])) {
...
function pickSourceGap(status, row, bucket) {
  ...
  if (status === 'watch') {
    ...
    return bucket % 2 === 0 ? 'span-inventory' : 'trace-source'
  }
...
function actionFor(profile, row, status, gap) {
  ...
  const dependency = row.dependency || 'the trace'

  if (status === 'critical') {
    return `Assign ${owner} to replay ${featureName} against ${dependency} and keep a fallback path ready for ${profile.appName}.`
  }
  if (status === 'opportunity') {
    return `Have ${owner} test whether ${featureName} can benefit from ${dependency} without changing the trace gate.`
  }
  if (status === 'watch') {
    return `Monitor ${featureName} for the next trace run and confirm the owner can smoke test the change.`
  }
  if (gap === 'owner-map') {
    return `Record ${owner} for ${featureName} so the next trace has a clear checker.`
  }
  return `Keep ${featureName} on the watch list and note the trace source in the team checklist.`
}
  ...
  return [
    {
      ...
      detail: profile.provider
        ? `The team has a named trace source: ${profile.provider}.`
        : 'No trace source has been mapped yet.',
      gap: profile.provider ? '' : 'trace-source',
    },
    {
      state: inventoryCount >= 6 ? 'ready' : inventoryCount >= 3 ? 'watch' : 'risk',
      label: 'Trace inventory',
      detail:
        inventoryCount >= 6
          ? `The matrix covers ${inventoryCount} trace rows.`
          : inventoryCount >= 3
            ? `The matrix has ${inventoryCount} rows, but 6 gives better coverage.`
            : 'The matrix is too small to expose the main trace edges.',
    ...
    },
    {
      ...
      detail:
        criticalCount <= 1
          ? 'The critical paths can be smoke-tested before the next trace run.'
          : 'The trace needs a replay or smoke plan before it is safe.',
...
function buildActions(profile, prompts, findings, readiness) {
  ...
  return [
    {
      id: 'map-source',
      title: 'Map the trace source',
      impact: 5,
      effort: 1,
      why: `Summarize ${profile.provider} into one trace brief before the team acts on it.`,
    },
    {
      id: 'assign-owners',
      ...
      why: criticalRows.length
        ? `Make ${criticalRows.length} critical trace rows explicit so the checklist has one owner per item.`
    ...
    {
      id: 'run-smoke',
      title: 'Run a trace smoke check',
      impact: 4,
      effort: 2,
      why: `Use the trace matrix to validate ${profile.workload} before the next trace lands.`,
    },
    {
      id: 'capture-upgrade',
    ...
    },
    {
      ...
      impact: 4,
      effort: 1,
      why: 'Give the team one page that lists the trace source, the matrix, the owners, and the next checks.',
    },
  ]
}
...
function buildSummary(profile, findings, actions, score) {
  ...
  const topAction = actions[0]?.title || 'Map the trace source'
  const highestPressure = profile.driftModels[0]?.name || profile.currentModel

  return {
    headline:
      score.overall >= 70
        ? 'The trace plan is manageable if the team keeps the matrix and owner checklist first.'
        : 'The trace still has a few high-risk edges that should be fixed before cutover.',
    traceWindow:
      criticalCount > 0
        ? 'This week'
        : watchCount > 2
    ...
    impactGap:
      profile.driftModels.length
        ...
        : 'No trace rows have been added yet, so the matrix is incomplete.',
    ownerGap:
      profile.driftModels.filter((row) => row.owner).length
        ? 'Owners are mapped for the trace checklist.'
        : 'The trace has no owner map yet.',
    topAction,
  }
}
