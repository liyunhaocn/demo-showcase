...
const output = path.join(showcaseRoot, 'demos', 'actions-parallelism-planner')
...
execFileSync(
  ...
  [
    ...
    '--format=iife',
    '--platform=browser',
    '--global-name=ActionsParallelismPlanner',
    `--outfile=${bundlePath}`,
  ],
  { stdio: 'inherit' },
)
...
console.log(`Built Actions Parallelism Planner static demo into ${path.relative(showcaseRoot, output)}`)
