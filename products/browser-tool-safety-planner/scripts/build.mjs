...
const output = path.join(showcaseRoot, 'demos', 'browser-tool-safety-planner')
...
execFileSync(
  ...
  [
    ...
    '--format=iife',
    '--platform=browser',
    '--global-name=BrowserToolSafetyPlanner',
    `--outfile=${bundlePath}`,
  ],
  { stdio: 'inherit' },
)
...
console.log(`Built Browser Tool Safety Planner static demo into ${path.relative(showcaseRoot, output)}`)
