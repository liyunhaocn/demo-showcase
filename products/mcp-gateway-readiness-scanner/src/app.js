...
function bindEvents() {
  ...
  elements.resetButton.addEventListener('click', () => {
    const confirmed = window.confirm('Reset demo data and clear the saved MCP Gateway Readiness Scanner brief from this browser?')
    if (!confirmed) return
    state.brief = null
    state.copyFallbackVisible = false
    clearErrors()
  ...
  elements.copyButton.addEventListener('click', async () => {
    ...
    if (result.ok) {
      state.copyFallbackVisible = false
      state.lastMessage = 'Markdown gateway brief copied to clipboard.'
    } else {
...
function generateFromForm() {
  ...
  submit.textContent = 'Building deterministic gateway brief...'
...
function renderReport() {
  ...
  replaceChildren(elements.scoreGrid, [
    metricCard(`${brief.score.overall}`, `Overall gateway readiness - ${scoreBand(brief.score.overall)}`),
    metricCard(`${brief.findings.length}`, 'gateway items'),
    metricCard(`${brief.score.impactCoverage}`, 'tool coverage'),
    metricCard(`${brief.score.ownerClarity}`, 'owner clarity'),
  ])

  replaceChildren(elements.summaryGrid, [
    summaryCard('Gateway snapshot', `${brief.profile.appName} comparing ${brief.profile.provider} and the tracked servers across ${brief.profile.region}.`),
    summaryCard('Gateway gap', brief.summary.impactGap),
...
function promptCard(brief, finding) {
  ...
  card.append(
    cell('Gateway item', prompt?.prompt || finding.promptId),
...
function setGenerateLabel() {
  const submit = elements.form.querySelector('button[type="submit"]')
  if (!submit.disabled) submit.textContent = state.brief ? 'Regenerate gateway brief' : 'Generate gateway brief'
}
