...
const output = path.join(showcaseRoot, 'demos', 'context-split-planner')
...
execFileSync(
  ...
  [
    ...
    '--format=iife',
    '--platform=browser',
    '--global-name=ContextSplitPlanner',
    `--outfile=${bundlePath}`,
  ],
  { stdio: 'inherit' },
)
...
console.log(`Built Context Split Planner static demo into ${path.relative(showcaseRoot, output)}`)
